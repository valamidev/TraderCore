...
class Accounts {
  ...
  async load_account_options(guid) {
    try {
      let [rows] = await pool.query("SELECT * FROM `account_options` WHERE owner_guid = ?;", [guid])
  ...
  async replace_options(guid, name, value) {
    try {
      await pool.query("REPLACE INTO `account_options` (`owner_guid`, `name`, `value`) VALUES (?,?,?);", [guid, name, value])
      return
    } catch (e) {
      logger.error("SQL error", e)
    }
