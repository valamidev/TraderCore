...
class LiveEmulator {
  ...
  async start() {
    try {
      ...
      for (let i = 0; i < this.simulations.length; i++) {
        let candledata = await tradepairs.get_candlestick(this.simulations[i].exchange, this.simulations[i].symbol, this.simulations[i].interval_sec, this.hot_start_limit)
  ...
  async reload_strategies() {
    try {
      ...
      for (let i = 0; i < all_simulations.length; i++) {
        ...
        if (_.findIndex(this.simulations, { guid: new_simulation.guid }) == -1) {
          let candledata = await tradepairs.get_candlestick(new_simulation.exchange, new_simulation.symbol, new_simulation.interval_sec, this.hot_start_limit)

          this.simulations.push(new_simulation)

          let new_index = this.simulations.length - 1

          this.simulations[new_index].emulator = new Emulator(this.simulations[new_index])

          this.simulations[new_index].first_advice = true

          promise_start.push(this.simulations[new_index].emulator.start(candledata))
          count++
        }
      }
  ...
  async update_loop() {
    try {
      ...
      logger.verbose(`Live strategies updated, count: ${this.simulations.length} , time: ${time} last_candle: ${this.simulations[0].emulator.last_update.time} `)
  ...
  async single_update(strategies_id) {
    try {
      ...
      let update_tick = await this.simulations[guid].emulator.update(candledata)
      ...
      if (update_tick > 0) {
        ...
        if (this.simulations[guid].emulator.last_advice !== this.simulations[guid].last_advice) {
          ...
          if (this.simulations[guid].first_advice == false) {
            this.save_advice(this.simulations[guid], this.simulations[guid].emulator.last_advice)
          } else {
            this.simulations[guid].first_advice = true
          }
        } else {
  ...
  async load_strategies() {
    try {
      let [rows] = await pool.query("SELECT `guid`,`symbol`,`exchange`,`strategy`,`strategy_config`,`interval_sec` FROM `trade_strategies` ORDER BY `guid` ASC;")
