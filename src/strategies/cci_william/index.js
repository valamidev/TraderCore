...
class Strategy {
  ...
  cci_signal() {
    ...
    if (
      this.BUF.cci_long[this.step] < -200 * this.mul_cci_n200 &&
      this.BUF.cci_short[this.step] < -100 * this.mul_cci_n100
    ) {
      signal = "BUY";
    }

    if (
      this.BUF.cci_long[this.step] > 200 * this.mul_cci_200 &&
      this.BUF.cci_short[this.step] > 100 * this.mul_cci_100
    ) {
      signal = "SELL";
    }
