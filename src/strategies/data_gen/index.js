...
class Strategy {
  ...
  async update(candledata) {
    try {
      ...
      if (this.step > this.minimum_history) {
        ...
        if (price_close < this.BUF.bb_upper[this.step] && price_close >= this.BUF.bb_middle[this.step]) zone = "high"
        if (price_close > this.BUF.bb_lower[this.step] && price_close < this.BUF.bb_middle[this.step]) zone = "low"
        ...
        // Buy
        if (price_close <= this.BUF.bb_lower[this.step] && this.BUF.rsi[this.step] <= 32 && this.trend.duration >= risk.persistence && this.advice != "BUY") {
          await this.BUY()
        }

        // Sell
        if (price_close >= this.BUF.bb_middle[this.step] && this.BUF.rsi[this.step] >= 42 && this.advice != "SELL") {
          await this.SELL()
        }
      }
    } catch (e) {
  ...
  risk() {
    ...
    let percent_of_price = (this.BUF.bb_middle[this.step] - this.BUF.bb_lower[this.step]) / this.BUF.candle_mid[this.step]
