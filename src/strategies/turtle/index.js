  ...
  async update(candledata) {
    try {
      ...
      if (this.step > this.minimum_history) {
        ...
        if (this.BUF.candle[this.step].high >= this.BUF.donc_mid[this.step].max && this.last_trade_profit < 0) {
          await this.BUY("20day")
        }

        // Entry -  The price breaks out of a 55-day Donchian channel.
        if (this.BUF.candle[this.step].high >= this.BUF.donc_long[this.step].max) {
          await this.BUY("55day")
        }

        // 20-day channel, you need to exit at the breakout of the opposite 10-day channel.
        if (this.BUF.candle[this.step].low <= this.BUF.donc_short[this.step].min && this.last_entry_type == "20day") {
          await this.SELL()
        }

        // 55-day,  you need to exit at the breakout of the opposite 20-day channel.
        if (this.BUF.candle[this.step].low <= this.BUF.donc_mid[this.step].min && this.last_entry_type == "55day") {
          await this.SELL()
        }
      }
    } catch (e) {
