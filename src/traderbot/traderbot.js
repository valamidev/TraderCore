...
class Traderbot {
  ...
  async start() {
    try {
      ...
      // Update tradeinfo useable for trade rounding
      this.tradeinfo = await trade_util.get_tradeinfo();
  ...
  async load_instances() {
    try {
      ...
      logger.verbose(
        `Tradebot instances loaded, count: ${this.trade_instances.length}`
      );
    } catch (e) {
  ...
  async api_update_trade_instance(
    instanceID,
    advice = { action: "IDLE", price: 1000000 }
  ) {
    try {
  ...
  async update_trade_instances() {
    try {
      ...
      this.strategy_advices = await this.get_trade_advice_db(
        this.last_advice_time
      );

      logger.verbose(
        `Trade advice length: ${
          this.strategy_advices.length
        } Last advice time: ${this.last_advice_time}`
      );
      ...
      if (this.strategy_advices.length != 0) {
        ...
        for (let i = 0; i < this.trade_instances.length; i++) {
          ...
          let strategy_advice = this.strategy_advices.filter(
            elem => elem.strategy_guid == trader.strategy_guid
          );
      ...
      logger.verbose(
        `Tradebot instances updated, count: ${this.trade_instances.length}`
      );
  ...
  async load_trade_instances_db() {
    try {
      let [rows] = await pool.query(
        "SELECT * FROM `account_trader_instances`;"
      );
  ...
  async get_trade_advice_db(time = 0) {
    try {
      let [rows] = await pool.query(
        "SELECT DISTINCT `trade_advice`.`symbol`, `trade_advice`.`exchange`, `trade_advice`.`strategy_guid`, `trade_advice`.`strategy`, `trade_advice`.`strategy_config`, `action`,  `time`, `asset`, `quote`, `close` FROM `trade_advice` JOIN tradepairs ON trade_advice.symbol = tradepairs.symbol WHERE time > ? ORDER BY `time` DESC;",
        [time]
      );
  ...
  async get_db_last_advice_time() {
    try {
      let [rows] = await pool.query(
        "SELECT time FROM `trade_advice` ORDER BY `trade_advice`.`time` DESC LIMIT 1;"
      );
